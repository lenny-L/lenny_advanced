...
if __name__ == "__main__":
    ...
    """
    # 文件输入输出
    with open("../temp_test/in.txt", "r") as fin:
        text = fin.read()

    word_and_freq = parse(text)

    with open("../temp_test/out.txt", "w") as fout:
    ...
    #
    # with open("../temp_test/out_readline.txt", "w") as fout:
    #     for word, freq in word_and_freq:
    #         fout.write('{} {}\n'.format(word, freq))

    import json
    params = {'symbol': '123456', 'type': 'limit', 'price': 123.4, 'amount': 23}
    params_str = json.dumps(params)  # 将obj序列化成json格式的str
    print('after json serialization')
    print('type of params_str = {}, params_str = {}'.format(type(params_str), params))
    original_params = json.loads(params_str)  # 将json格式的str反序列化成原obj
